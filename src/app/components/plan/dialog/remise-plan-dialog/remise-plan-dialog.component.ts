...
export class RemisePlanDialogComponent implements OnInit {
  ...
  constructor(private router: Router, @Inject(MAT_DIALOG_DATA) public data: any, private planService: PlanService, public dialogRef: MatDialogRef<RemisePlanDialogComponent>) { }
  ...
  verificationRemise() {
    this.remise = this.reg.test(this.remiseInput.nativeElement.value) ? parseInt(this.remiseInput.nativeElement.value, 10) : 0;
    if (this.remise === 0) {
      this.isFilled = false;
    } else {
      this.isFilled = true;
